...
# globals
RAINBOW_EXPECTED_HEIGHT = 256
RAINBOW_EXPECTED_WIDTH = 512
...
def read(file_list, workers=1, first_frame=False, no_metadata=False, quiet=False):
    ...
    if (workers > 1):
        ...
        data = []
        for f in file_list:
            data.append(__rainbow_readfile_worker(
                f,
                first_frame=first_frame,
                no_metadata=no_metadata,
                quiet=quiet,
    ...
    total_num_frames = 0
    image_height = RAINBOW_EXPECTED_HEIGHT
    image_width = RAINBOW_EXPECTED_WIDTH
    ...
    images = np.empty([image_height, image_width, total_num_frames], dtype=RAINBOW_DT)
...
def __rainbow_readfile_worker(file, first_frame=False, no_metadata=False, quiet=False):
    # init
    images = np.array([])
    metadata_dict_list = []
    is_first = True
